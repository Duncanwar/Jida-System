...
export async function loginByRole(role: LoginRole, email: string, password: string) {
    ...
    // Commented out to prevent 404 errors from missing backend
    /*
    try {
...
export async function loginAuthor(email: string, password: string) {
    ...
    */
    ...
    // Dummy response to prevent 404
    return {
        token: 'demo-author-token',
        user: { email, role: 'author' },
    };
}
...
export async function registerAuthor(data: {
    ...
    */
    ...
    return {
        token: 'demo-author-token',
        user: { email: data.email, role: 'author', name: data.name },
    };
}
